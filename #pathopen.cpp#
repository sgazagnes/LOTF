...
void TrackZ_CoordinatesDistNorm(CoordGrid &hitMap, std::vector<TrackObject*>* trks)
{
  if( (trks == 0) || (trks->size() == 0) ) {
    std::cerr << "<ERROR> Input track list is empty. Terminating program.\n";
    //exit(EXIT_FAILURE);
    return;
  }
  std::cout << "<INFO> Determining the z coordinates for points in tracks (Dist Norm).\n"
	    << "\t<-I-> total number of input tracks = " << trks->size()
	    << '\n';
  ...
  for(size_t tr = 0; tr < trks->size(); ++tr) {
    ...
    std::cout << "<DEBUG> Last z value (dZ)" << lastVirtualZCoord
	      << ", Max layes = "    << maxLayer;
#endif
...
#if(REORDER_AND_NORM_NODES_FOR_Z > 0)
#if(Z_DETERMINATION_DEBUG > 0)
    std::cerr << "\tDetermine Z. Reordering nodes by layer.\n";
#endif
...
#else
#if(Z_DETERMINATION_DEBUG > 0)
    std::cerr << "\tDetermine Z normalized by prev Dist.\n";
#endif
    /*
