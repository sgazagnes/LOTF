...
void PlotConnectedComponents( std::string const& Infile = "Track3DPositions.root",
			      ...
			      )
{
  ...
  for(size_t ev = 0; ev < nEv; ++ev) {
    ...
    for(size_t i = 0; i < nComp; i++) {
      ...
      oriN << i;
      tpName = "Comp_Evt_" + evN.str() + "_" + oriN.str();
  ...
  TCanvas *compCanvas = new TCanvas("AllComponentsCanvas", "AllComponents", ww, hh );
